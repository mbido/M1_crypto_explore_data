...
document.addEventListener(
    'DOMContentLoaded', () => {
      ...
      const updateWorldsButton = document.getElementById('updateWorldsButton');
      ...
      async function fetchData(url, options = {}) {
        ...
        try {
          ...
          if (contentType && contentType.includes('application/json')) {
            ...
            const textResponse = await response.text();  // Get text for context
            console.error(
      ...
      function renderUserDetail(data) {
        ...
        // --- MODIFICATION : Ajout des IDs et du bouton dans positionHtml
        // ---
        let positionHtml =
        ...
        if (last_position) {
          positionHtml = `
              ...
              last_position.world_ID || 'N/A'}</span></p>
                <p><strong class="text-gray-600 dark:text-slate">Location:</strong> <span id="live-location-value" class="font-mono text-xs">${
              last_position.location || 'N/A'}</span></p>
                <p><strong class="text-gray-600 dark:text-slate">Salle:</strong> <span id="live-room-value">${
              last_position.room || 'N/A'}</span></p>
                <p><strong class="text-gray-600 dark:text-slate">Vu le:</strong> <span id="live-timestamp-value">${
              formatDate(last_position.created_at)}</span></p>
                <div class="mt-3 flex items-center">
                    <button id="checkLiveLocationBtn"
                            data-world-id="${last_position.world_ID}"
                            class="py-1 px-3 text-xs bg-blue-500 hover:bg-blue-600 dark:bg-green dark:hover:opacity-90 text-white dark:text-navy font-medium rounded transition duration-150 ease-in-out">
                        <i class="fas fa-sync-alt mr-1"></i> Vérifier Live
                    </button>
                    <span id="live-status-indicator" class="ml-2 text-xs"></span>
                </div>
            `;
        }
        // --- FIN MODIFICATION positionHtml ---
        ...
        // --- MODIFICATION : Appel pour attacher l'écouteur ---
        attachLiveLocationListener(last_position);
        // --- FIN MODIFICATION ---

        hideLoading();
      }  // --- FIN renderUserDetail ---

      function attachLiveLocationListener(originalPositionData) {
        const checkButton = document.getElementById('checkLiveLocationBtn');
        const locationSpan = document.getElementById('live-location-value');
        const roomSpan = document.getElementById('live-room-value');
        const timestampSpan = document.getElementById('live-timestamp-value');
        const statusIndicator =
            document.getElementById('live-status-indicator');

        // Sauvegarder la valeur initiale du timestamp si elle existe (pour
        // la restauration en cas d'erreur)
        const initialTimestampText = timestampSpan ?
            (originalPositionData ?
                 formatDate(originalPositionData.created_at) :
                 'N/A') :
            null;

        if (checkButton && locationSpan && roomSpan && timestampSpan &&
            statusIndicator) {
          // Cloner et remplacer pour éviter les écouteurs multiples
          const newButton = checkButton.cloneNode(true);
          checkButton.parentNode.replaceChild(newButton, checkButton);

          newButton.addEventListener('click', async () => {
            const worldId = newButton.dataset.worldId;
            if (!worldId) {
              statusIndicator.textContent = 'Erreur: World ID manquant.';
              statusIndicator.className =
                  'ml-2 text-xs text-red-500 dark:text-red-400';
              return;
            }
            ...
            newButton.innerHTML =
                '<i class="fas fa-spinner fa-spin mr-1"></i> Vérif...';
            statusIndicator.textContent = 'Chargement...';
            statusIndicator.className =
                'ml-2 text-xs text-gray-500 dark:text-slate italic';
            // Réinitialiser les styles des spans au cas où une précédente
            // vérification avait réussi/échoué
            locationSpan.className = 'font-mono text-xs';  // Style initial
            roomSpan.className = '';                       // Style initial
            timestampSpan.className = '';                  // Style initial

            console.log(`[LiveCheck] ${worldId}: Démarrage vérification...`);
            try {
              const result = await fetchData(`/world/${
                  encodeURIComponent(
                      worldId)}/live_location`);  // Utilisation de l'URL de
                                                  // l'API
              console.log(`[LiveCheck] ${worldId}: Réponse API reçue`, result);

              if (result && result.success) {
                console.log(`[LiveCheck] ${worldId}: Succès.`);
                locationSpan.textContent = result.location || 'Inconnue';
                locationSpan.className =
                    'font-semibold text-green-600 dark:text-green';  // Style
                                                                     // succès

                roomSpan.textContent = result.room || 'Inconnue';
                roomSpan.className =
                    'font-semibold text-green-600 dark:text-green';  // Style
                                                                     // succès

                timestampSpan.textContent =
                    `Live @ ${formatDate(result.checked_at)}`;
                ...
                statusIndicator.className =
                    'ml-2 text-xs text-green-600 dark:text-green font-bold';
              }


            } catch (error) {  // Erreur de fetch OU erreur lancée par fetchData
                               // pour non-OK status
              console.error(`[LiveCheck] ${worldId}: Erreur attrapée:`, error);

              // Restaurer les valeurs initiales d'abord
              locationSpan.textContent =
                  originalPositionData?.location || 'N/A';
              locationSpan.className = 'font-mono text-xs';  // Style initial
              roomSpan.textContent = originalPositionData?.room || 'N/A';
              roomSpan.className = '';  // Style initial
              timestampSpan.textContent =
                  initialTimestampText;  // Restaurer le timestamp initial via
                                         // la variable
              timestampSpan.className = '';  // Style initial

              // --- MODIFICATION DU CATCH ---
              // Vérifier si c'est l'erreur 404 spécifique de notre API
              // live_location
              if (error.status === 404 && error.payload &&
                  (error.message.includes('Monde inactif') ||
                   error.payload.error?.includes('inactif') ||
                   error.payload.details?.includes('Unknown world_id'))) {
                // C'est notre cas "Monde inactif"
                console.log(`[LiveCheck] ${
                    worldId}: Erreur 404 identifiée comme Monde Inactif.`);
                const errorMessage = error.payload.error ||
                    error.message;  // Utiliser le message d'erreur du payload
                                    // si possible
                const errorDetails = error.payload.details || '';

                statusIndicator.innerHTML =
                    `<i class="fas fa-times-circle mr-1"></i> Monde inactif`;
                statusIndicator.className =
                    'ml-2 text-xs text-red-500 dark:text-red-400 font-bold flex items-center';
                statusIndicator.title = errorMessage +
                    (errorDetails ? ` (${errorDetails})` :
                                    '');  // Tooltip avec détails

              } else {
                // C'est une autre erreur (réseau, serveur 500, autre 4xx, etc.)
                console.log(
                    `[LiveCheck] ${worldId}: Erreur générique ou réseau.`);
                statusIndicator.innerHTML =
                ...
                statusIndicator.title =
                    error.message;  // Afficher le message de l'erreur attrapée
              }
              console.log(
                  `[LiveCheck] ${worldId}: Indicateur mis à jour (Catch): ${
                      statusIndicator.innerHTML}`);
              // --- FIN MODIFICATION DU CATCH ---


            } finally {
              console.log(`[LiveCheck] ${
                  worldId}: Bloc Finally exécuté.`);  // Log Finally
              // Réactiver le bouton après un court délai
              setTimeout(() => {
                if (newButton) {  // Vérifier si le bouton existe toujours
          ...
          // Log seulement si le bouton existe mais pas les spans (indique
          // un problème potentiel dans le HTML généré)
          if (document.getElementById('checkLiveLocationBtn') &&
              (!locationSpan || !roomSpan || !timestampSpan ||
               !statusIndicator)) {
            console.warn(
                'Le bouton checkLiveLocationBtn existe mais certains spans (location/room/timestamp/status) sont manquants.');
          }
          // Sinon, c'est normal si last_position n'existait pas, le bouton
          // n'est pas rendu.
        }
      }
        ...
        function setupSearchableDropdown(inputId, dropdownId, valueId) {
          ...
          dropdownElement.addEventListener('mousedown', (e) => {
            ...
            if (item) {
              ...
              clearTimeout(blurTimeout);  // Prevent blur from hiding too soon
      ...
      async function executeSandboxCommand(
          ...
          try {
            if (input.tagName === 'TEXTAREA' || typeHint.includes('json') ||
              ...
              if (value !== '') {
                ...
                if (isNaN(numValue)) {
                  setError(`Erreur: Valeur numérique invalide pour '${name}'.`);
                  return;
                }
              ...
              if (value === 'true')
                ...
                value = null;  // Optional unselected boolean -> null
            ...
            if (e instanceof SyntaxError && input.tagName === 'TEXTAREA') {
              setError(`Erreur: JSON invalide pour le paramètre '${name}'.`);
            } else {
        ...
        try {
          console.log(`Executing ${methodName} with processed params:`, params);
            ...
            commandsWithParams
                ...
                <div class="sandbox-card bg-white dark:bg-light-navy p-4 rounded-lg shadow mb-4 cursor-pointer" data-method-name="${
                    ...
                    <p class="text-xs text-gray-500 dark:text-slate mb-2 pointer-events-none">${
                        cmd.description || ''}</p> <!-- Display description -->
        ...
        if (searchInput && commandList && noResultsMessage) {
          searchInput.addEventListener('input', (e) => {
            ...
            cards.forEach(card => {
              ...
              const description =
                  card.querySelector('.text-xs')?.textContent.toLowerCase() ||
      ...
      function displayReachableRooms(rooms, container, statusDiv) {
        ...
        rooms.sort((a, b) => {
          ...
          const nameA =
              (a.name || a.id || '').toLowerCase();  // || '' pour éviter erreur
                                                     // si null/undefined
        ...
        rooms.forEach(room => {
          ...
          card.textContent =
              room.name || room.id;  // Affiche le nom si disponible, sinon l'ID
      ...
      async function handleRoomCardClick(event) {
        ...
        const roomsContainer = document.getElementById(
            'reachableRoomsContainer');          // Référence au conteneur des
                                                 // cartes
        const worldId = currentTeleportWorldId;  // Utilise l'ID du monde stocké
                                                 // lors du dernier DFS
        ...
        try {
          ...
          if (result.success) {
            const newLocation = result.new_location;  // Détails de la nouvelle
                                                      // position (si fournie)
            ...
            if (newLocation) {
              ...
              successMsg += `<br><span class="text-xs">Nouvelle position ID: ${
                  newLocation.id}</span>`;
            }
      ...
      function attachTeleportElements() {
        ...
        const noResultsMessage = document.getElementById('teleportNoResults');
        ...
        if (findRoomsBtn && roomsContainer && statusDiv && worldIdInput &&
          ...
          findRoomsBtn.parentNode.replaceChild(newFindRoomsBtn, findRoomsBtn);
          ...
          newFindRoomsBtn.addEventListener('click', async () => {
            ...
            try {
              const rooms = await fetchData(
                  `/reachable_rooms?world_id=${encodeURIComponent(worldId)}`);
      ...
      async function handleRouteChange() {
        ...
        // currentTeleportWorldId = null;

        try {
          if (hash === '#dashboard') {
            const data = await fetchData('/stats');  // API path correction
            renderDashboard(data);
            attachTeleportElements();
          } else if (hash === '#users') {
            currentSortKey = 'username';
            currentSortDirection = 'asc';
            const data = await fetchData('/users');  // API path correction
            renderUserList(data);
          } else if (hash.startsWith('#user/')) {
            const username = decodeURIComponent(hash.substring(6));
            const data =
                await fetchData(`/user/${username}`);  // API path correction
            renderUserDetail(
                data);  // renderUserDetail appelle attachLiveLocationListener
          } else if (hash === '#compare') {
            const usersData = await fetchData('/users');  // API path correction
            renderCompareForm(usersData);
          } else if (hash === '#sandbox') {
            const commandsWithParams =
                await fetchData('/sandbox/commands');  // API path correction
        ...
        navLinks.forEach(link => {
          ...
          try {
            ...
            if (decodedHash.startsWith('#user/') && decodedHref === '#users') {
              isActive = true;  // Highlight 'Users' link for user detail pages
            } else {
          ...
          link.classList.toggle('dark:bg-lightest-navy', isActive && isDark);
          link.classList.toggle('font-semibold', isActive);

          if (isActive) {
            link.classList.add(isDark ? 'dark:text-green' : 'text-gray-900');
          } else {
        ...
        updateDbButton.addEventListener('click', async () => {
          ...
          try {
            ...
            if (result && result.success) {
              ...
              const errorMessage =
                  result?.error || 'Erreur inconnue renvoyée par le serveur.';
              updateStatusDiv.innerHTML =
        ...
        updateWorldsButton.addEventListener('click', async () => {
          ...
          try {
            ...
            if (result && result.success) {
              ...
              const errorMessage = result?.error || 'Erreur inconnue serveur.';
              updateWorldsStatusDiv.innerHTML =
